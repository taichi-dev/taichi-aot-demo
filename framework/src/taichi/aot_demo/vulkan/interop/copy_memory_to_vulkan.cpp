#include "taichi/aot_demo/interop/copy_memory_to_vulkan.hpp"
...
struct DeviceMemoryHandle {
  ...
  DeviceMemoryHandle(DeviceMemoryHandle &&b)
...
void copy_memory_vulkan_to_vulkan(ti::Runtime &dst_runtime,
  ...
  ti_export_vulkan_memory(src_runtime, src_vulkan_ndarray.memory().memory(),
                          &src_vulkan_interop_info);
  ...
  copyBuffer(vk_device, cmd_pool, graphics_queue, src_buffer, dst_buffer,
             buffer_size);
}
...
void copy_memory_cpu_to_vulkan(ti::Runtime &runtime, ti::Memory &vulkan_ndarray,
                               ti::Runtime &cpu_runtime,
                               ti::Memory &cpu_ndarray) {
#ifdef TI_WITH_CPU
  // Get Interop Info
  TiVulkanMemoryInteropInfo vulkan_interop_info;
  ti_export_vulkan_memory(runtime.runtime(), vulkan_ndarray.memory().memory(),
                          &vulkan_interop_info);

  TiCpuMemoryInteropInfo cpu_interop_info;
  ti_export_cpu_memory(cpu_runtime, cpu_ndarray.memory().memory(),
                       &cpu_interop_info);
  ...
  createBuffer(vk_device, physical_device, buffer_size,
  ...
  copyBuffer(vk_device, cmd_pool, graphics_queue, staging_buffer,
             vulkan_interop_info.buffer, buffer_size);
#else
  throw std::runtime_error(
      "Unable to perform copy_from_cpu<T>() with TI_WITH_CPU=OFF");
#endif // TI_WITH_CPU
}
...
CUexternalMemory
import_cuda_memory_object_from_handle(const DeviceMemoryHandle &handle,
...
void copy_memory_cuda_to_vulkan(GraphicsRuntime &runtime,
  ...
  ti_export_cuda_memory(cuda_runtime, cuda_ndarray.memory().memory(),
                        &cuda_interop_info);
  ...
  CUdeviceptr dst_cuda_ptr =
      reinterpret_cast<CUdeviceptr>(map_buffer_onto_external_memory(
          externalMem, alloc_offset, vulkan_interop_info.size));
  CUdeviceptr src_cuda_ptr =
      reinterpret_cast<CUdeviceptr>(cuda_interop_info.ptr);

  cuMemcpyDtoD_v2(dst_cuda_ptr, src_cuda_ptr, vulkan_interop_info.size);
#else
  throw std::runtime_error(
      "Unable to perform copy_from_cuda<T>() with TI_WITH_CUDA=OFF");
#endif // TI_WITH_CUDA
}
...
struct OpenglMemoryObject {
  ...
  OpenglMemoryObject(GLuint memory_obj, GLuint buffer)
      : memory_obj(memory_obj), buffer(buffer) {}
  OpenglMemoryObject(const OpenglMemoryObject &) = delete;
  OpenglMemoryObject(OpenglMemoryObject &&b)
      : memory_obj(std::exchange(b.memory_obj, 0)),
        buffer(std::exchange(b.buffer, 0)) {}
  ~OpenglMemoryObject() {
...
OpenglMemoryObject
import_opengl_memory_object_from_handle(const DeviceMemoryHandle &handle,
...
#ifdef WIN32
  glImportMemoryWin32HandleEXT(memory_obj, size,
                               GL_HANDLE_TYPE_OPAQUE_WIN32_EXT, handle.handle);
#else
  glImportMemoryFdEXT(memory_obj, size, GL_HANDLE_TYPE_OPAQUE_FD_EXT,
                      handle.fd);
...
void copy_memory_opengl_to_vulkan(ti::Runtime &runtime,
  ...
  size_t mem_size = alloc_offset + alloc_size;
  auto handle = get_device_mem_handle(vertex_buffer_mem, vk_device);
  OpenglMemoryObject memory_obj =
      import_opengl_memory_object_from_handle(handle, alloc_offset, alloc_size);
...
#else
  throw std::runtime_error(
      "Unable to perform copy_from_opengl<T>() with TI_WITH_OPENGL=OFF");
#endif // TI_WITH_OPENGL
}
