...
GraphicsTaskState::GraphicsTaskState(
  ...
  VkDevice device = renderer_state->device_;
  ...
  res = vmaCreateBuffer(renderer_state->vma_allocator_, &ubbci, &aci, &uniform_buffer,
    &uniform_buffer_allocation, nullptr);
  check_vulkan_result(res);

  void* ub;
  res = vmaMapMemory(renderer_state->vma_allocator_, uniform_buffer_allocation, &ub);
  check_vulkan_result(res);

  std::memcpy(ub, config.uniform_buffer_data, config.uniform_buffer_size);
  vmaUnmapMemory(renderer_state->vma_allocator_, uniform_buffer_allocation);
  ...
  for (size_t i = 0; i < config.resources.size(); ++i) {
    ...
    case L_GRAPHICS_TASK_RESOURCE_TYPE_BUFFER:
    {
      const TiVulkanMemoryInteropInfo &vmii = renderer_state->export_ti_memory(
          renderer->renderer_runtime(), *resource.shadow_buffer);
      check_taichi_error();
    ...
    case L_GRAPHICS_TASK_RESOURCE_TYPE_TEXTURE:
    {
      const TiVulkanImageInteropInfo &viii = renderer_state->export_ti_image(
          renderer->renderer_runtime(), *resource.shadow_texture);
      check_taichi_error();
      ...
      dii.sampler = renderer_state->sampler_;
  ...
  gpci.renderPass = renderer_state->render_pass_;
...
GraphicsTaskState::~GraphicsTaskState() {
  RendererState *renderer_state = renderer_->state();

  VkDevice device = renderer_state->device_;
  VmaAllocator vma_allocator = renderer_state->vma_allocator_;
...
void GraphicsTask::update() {
  const GraphicsTaskConfig &config = state_->config_;
