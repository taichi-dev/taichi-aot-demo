...
ShadowBuffer::ShadowBuffer(const std::shared_ptr<Renderer> &renderer,
  ...
  client_memory_ = ti::Memory(renderer_->client_runtime_, client_memory, client_memory.size(), false);
}
...
void ShadowBuffer::copy_from_vulkan_(const ti::Memory &src) {
  ti::Runtime &client_runtime = renderer_->client_runtime();

  assert(client_runtime.arch() == TI_ARCH_VULKAN);
  client_runtime.copy_memory_device_to_device(memory_.slice(), src.slice());
}

void ShadowBuffer::copy_from_cpu_(const ti::Memory &src) {
#if TI_WITH_CPU
  ti::Runtime &client_runtime = renderer_->client_runtime();
  ti::Runtime &runtime = renderer_->client_runtime();

  TiCpuMemoryInteropInfo cmii{};
  ti_export_cpu_memory(client_runtime, src.memory(), &cmii);

  ti::Memory staging_buffer = runtime.allocate_memory(memory_.size(), true);
  staging_buffer.write(cmii.ptr, cmii.size);

  runtime.copy_memory_device_to_device(memory_.slice(), staging_buffer.slice());
#endif // TI_WITH_CPU
}
...
#endif // TI_WITH_CUDA
void ShadowBuffer::copy_from_cuda_(const ti::Memory &src) {
#ifdef TI_WITH_CUDA
  ...
  ti_export_cuda_memory(cuda_runtime, src.memory(), &cuda_interop_info);
...
void ShadowBuffer::copy_from_opengl_(const ti::Memory &src) {
#ifdef TI_WITH_OPENGL
  ti::Runtime &runtime = renderer_->runtime_;
  ...
  ti_export_opengl_memory(opengl_runtime.runtime(),
                          src.memory(), &opengl_interop_info);
...
void ShadowBuffer::update() {
  switch (renderer_->client_runtime_.arch()) {
    case TI_ARCH_VULKAN:
      copy_from_vulkan_(client_memory_);
      break;
    case TI_ARCH_X64:
    case TI_ARCH_ARM64:
      copy_from_cpu_(client_memory_);
      break;
    case TI_ARCH_CUDA:
      copy_from_cuda_(client_memory_);
      break;
    case TI_ARCH_OPENGL:
      copy_from_opengl_(client_memory_);
      break;
    default:
      assert(false);
  }
