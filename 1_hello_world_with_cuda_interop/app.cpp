...
struct App1_hello_world_with_cuda_interop : public App {
  // Runtime/Ndarray to perform computations
  ti::Runtime cuda_runtime;
  ti::NdArray<float> cuda_points;
  ...
  virtual AppConfig cfg() const override final {
    AppConfig out {};
    out.app_name = "1_hello_world_with_cuda_interop";
    return out;
  }
  virtual void initialize() override final{
    // Prepare Ndarray to store computation results
    cuda_runtime = ti::Runtime(TiArch::TI_ARCH_CUDA);
    cuda_points = cuda_runtime.allocate_ndarray<float>({3}, {2}, true);
  ...
  virtual bool update() override final {
    ...
    // Store the computation results to "cuda_points"
    std::vector<float> points_data {
      -0.5f, -0.5f,
       0.0f, 0.0f,
       0.5f, 0.5f,
    };
    cuda_points.write(points_data);
    ...
    // Copy data from "cuda_points" to "render_points"
    InteropHelper<float>::copy_from_cuda(F.runtime(), render_points, cuda_runtime, cuda_points);
...
std::unique_ptr<App> create_app() {
  return std::unique_ptr<App>(new App1_hello_world_with_cuda_interop);
}
